...
int classify(LecunDataType train, LecunDataType labels, LecunDataType test, LecunDataType *result, int K){
  ...
  DistanceType *dists = (DistanceType *) calloc(train.dimensions[0], sizeof(DistanceType));
    //dimensions[1] = linhas e dimensions[2] = colunas;
  ...
  for(r=0; r<test.dimensions[0]; r++){
    for(c=0; c<train.dimensions[0]; c++){
	 ...
	 ... */      
        dists[c].data = distance(&test.data[r*n_pixels+c], &train.data[r*n_pixels+c], n_pixels);
        dists[c].idx = c;
    }
    ...
    for(c=0; c<NCLASSES; c++){
	 ...
	 ... */
        if(class_hist[count] > class_hist[c])
            count = c;
    }
...
int main(int nargin, char **argv){
  ...
  if((nargin < 6) || (nargin > 7)){    
    ...
    printf("\tgt:            path of file containing ground truth labels for test set.\n");
    ...
    printf("  gcc mnist.c lecun2img.c\n");
    printf("and use it:\n");
    printf("  ./a.out train-images-idx3-ubyte train-labels-idx1-ubyte t10k-images-idx3-ubyte results-idx1-ubyte 3 t10k-labels-idx1-ubyte\n");
