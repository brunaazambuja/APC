...
int allocate_img(Image *img, unsigned int channels, unsigned int rows, unsigned int cols, unsigned char max){
  ...
  img->cols = cols;
  img->max = max;
  img->data = malloc(img->chns * sizeof(unsigned char **)); // para cada cor aloca linhas e colunas
  for(chn=0; chn<channels; chn++){
    img->data[chn] = malloc(img->rows * sizeof(unsigned char *)); // aloca as linhas
    for(row=0; row < img->rows; row++)
      img->data[chn][row] = calloc(img->cols, sizeof(unsigned char)); // p cada linha, aloca tds as colunas
  }
  return(1);
}
...
int load_img(Image *img){
  ...
  if(magic_number==2) /* Grey image */ // n canais é 1
    channels = 1;
  else
    if(magic_number==3) /* Colour image */ // n de canais é 3
      channels = 3;
