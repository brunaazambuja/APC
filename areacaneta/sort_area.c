...
int main(int nargin, char **argv){
  LecunDataType input;
  unsigned int option, n_pixels, n_samples, n, c;
  DistanceType *areas;
  unsigned char *aux;
  ...
  n_pixels = input.dimensions[1]*input.dimensions[2];  /* tamanho da imagem (28x28);*/
  ...
  /* Write your own code here.
    ...
    aux = (unsigned char *) calloc(input.total_dims, input.data_bytes);
     /*7. For each sample n
	    ...
	    areas[n].idx to locate the sample of input.data and copy it to aux.
	  The same hint in step 2 will help to access locations in both 
	    input.data and aux.*/
    for (n = 0; n < n_samples; n++){     /*((n*n_pixels+c)) -- total_dims = vetor de 3 (qts imgs*linhas*cols);*/
      for(c = 0; c < n_pixels; c++){
     ...
     /*8. Now that aux has been sorted using idx, copy all its contents 
        back to input.data.*/
    for (n = 0; n < n_samples*n_pixels; n++){
      input.data[n] = aux[n];
    }
     /*9. free(aux)*/
    free(aux);
