...
int read_LecunData(LecunDataType *lecun){
  ...
  if(!fp){
    if(VERBOSE)  printf("Could not open %s for reading.\n", lecun->filename);
    return 1;
  }
  if(VERBOSE) printf("Reading IDX file %s\n", lecun->filename);
  if(fread(&magic_number, sizeof(magic_number), 1, fp)!=1){
    printf("Error reading magic number.\n");
    fclose(fp);
    return 2;
  }
  if(VERBOSE) printf("magic_number=%X\n", magic_number);
  lecun->data_type = (magic_number >> 2*8) & 0x00FF;
  if(VERBOSE) printf("data_type=%x\n", lecun->data_type);
  lecun->n_dimensions = magic_number >> 3*8;
  if(VERBOSE) printf("n_dimensions=%d\n", lecun->n_dimensions);
  ...
  for(d=0; d<lecun->n_dimensions; d++){    
    ...
    if(VERBOSE) printf("dimensions[%d]=%d\n", d, lecun->dimensions[d]);
  }

  if(VERBOSE) printf("total_dims=%lu\n", lecun->total_dims);
  ...
  if(VERBOSE) printf("data_bytes=%d\n", lecun->data_bytes);
...
double distance(unsigned char *x1, unsigned char *x2, int dims){
  ...
  for(c=0; c<dims; c++){
    ...
    d += fabs((double) x1[c] - (double) x2[c]);
  }
  if(VERBOSE > 3) printf("\n");
  return d;
}
...
void swap_lecun_imgs(LecunDataType *input, int a, int b){
  unsigned char *aux = (unsigned char *) calloc(input->total_dims, input->data_bytes);
  int n_pixels = input->dimensions[1]*input->dimensions[2];
  int c;
  for(c=0; c<n_pixels; c++){
    aux[c] = input->data[a*n_pixels+c];
    input->data[a*n_pixels+c] = input->data[b*n_pixels+c];
    input->data[b*n_pixels+c] = aux[c];
  }
  free(aux);
}
